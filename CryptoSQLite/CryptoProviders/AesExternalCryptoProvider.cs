...
namespace CryptoSQLite.CryptoProviders
{
    ...
    internal class Aes : IDisposable
    {
        ...
        // Invers SBOX AES
        private readonly byte[] _sBoxReverse =
        {
        ...
        private byte[,] InvSubBytes(byte[,] state)
        {
            ...
            for (var i = 0; i < 4; i++)
                for (var j = 0; j < 4; j++)
                    toRet[i, j] = _sBoxReverse[state[i, j]];
            return toRet;
        }
        ...
        // Functions of cycle row rotation (right) in state matrix
        ...
        // Function of columns permutation in state matrix
        private byte[,] InvMixedColumns(byte[,] state)
        {
            var toRet = new byte[4, 4];
            for (var row = 0; row < 4; row++)
            {
                ...
                {
                    for (var i = 0; i < 4; i++)
                        toRet[row, col] ^= PolinomialMultilay(_matrixForInverseMixedColumns[row, i], state[i, col]);
                }
            }
            return toRet;
        }
