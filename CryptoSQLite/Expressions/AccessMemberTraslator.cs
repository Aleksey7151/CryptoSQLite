...
{
    internal class AccessMemberTraslator
    {
        private static string _columnName;
        private static bool _isEncrypted;
        private static string _propertyName;
        public static string GetColumnName<TTable>(Expression<Func<TTable, object>> accessExpression, string tableName, ICollection<ColumnMap> mappedColumns, out bool isEncrypted, out string propertyName)
        {
        ...
        private static Expression TranslateExpression(Expression expression, string tableName, ICollection<ColumnMap> mappedColumns)
        {
            if (expression == null)
                return null;
        ...
        private static Expression TranslateUnaryExpression(UnaryExpression unaryExp, string tableName, ICollection<ColumnMap> mappedColumns)
        {
        ...
        private static Expression VisitLambda(LambdaExpression lambda, string tableName, ICollection<ColumnMap> mappedColumns)
        {
            ...
            Expression body = TranslateExpression(lambda.Body, tableName, mappedColumns);

            return body != lambda.Body ? Expression.Lambda(lambda.Type, body, lambda.Parameters) : lambda;
        }

        private static Expression TranslateMemberAccess(MemberExpression memberExp, string tableName, ICollection<ColumnMap> mappedColumns)
        {
